...
//			return selectCourseRes2;
	...
	// [上課星期 防呆]
	private boolean classDay(String classDay) {
	...
	// [時間 防呆]
	private LocalTime classTime(String allTime) { // (String allTime)為了對應Postman的req寫法格式!!式自訂義名稱，與上方設置的參數無關
		try {
			// 把字串轉換成LocalTime
	...
	public SelectCourseRes updateCours(String coursCode, String cours, String classDay, String classTime, String recess,
		...
		if (!classDay(classDay) || classtimeLocalTime == null || recessLocalTime == null
...
//			updateClass.setUnits(units);
			...
			schoolDao.save(school);
			// 課程修改成功
			return new SelectCourseRes(school, SelectCourseRtnCode.REVISE_SUCCESSFUL.getRtnmessage()); // 為了讓JSON印出
		}
	}
	...
	public SelectCourseRes classnameQuery(String course) { // 查詢的課程代碼是否符合DB資料庫的代碼 符合就往下跑
		...
		return new SelectCourseRes(coursNameQuery, SelectCourseRtnCode.QUERY_SUCCESSFUL.getRtnmessage()); // 查詢成功
																											// 以上為課程資訊
	}
	/* ====================================== */
	...
	public SelectCourseRes addStudentSelectCourse(String studentId, String studentName) {
		...
		Optional<Student> studentOp = studentDao.findById(studentId); // 判斷DB是否有學號
		if (studentOp.isPresent()) { // 有， 如果學號存在
	...
	public SelectCourseRes updateStudentSelectCourse(String studentId, String studentName) {
		...
		if (!studentOp.isPresent()) { // 確認此學生資訊是否存在
...
//		}
		...
		Student student = confirmStudentId.get(); // 有 --> 透過學號(pk)取得此學生的所有個人資訊
		...
		List<School> courseCodeList = schoolDao.findAllByCoursecodeIn(coursCode); // courscode：從PK取得課程資訊
		for (School item : courseCodeList) { // 從課程代碼抓取課程名稱
			courseList.add(item.getCourse());
			courseSet.add(item.getCourse());
			allUnits = allUnits + item.getUnits(); // 從課程代碼抓取學分資訊
//			allUnits += keepAllCourseCodeContent.getUnits();              
			item.getClassDay(); // 從課程代碼抓取上課星期
			item.getClassTime(); // 從課程代碼抓取上課時間
			item.getRecess(); // 從課程代碼抓取下課時間
		}
		...
		// ==============================================================
		// 不得衝堂
		SelectCourseRes res = checkCourseConflict(coursCode); //將輸入的課程代碼帶入課程衝突的方法去判斷
		if (res != null) {       //如果判斷結果不等於null，代表有衝堂
			return res;          //所以回傳衝堂的提示訊息
		}
		...
		student.setStudentClassCode(courscodeStr);
		studentDao.save(student);
		return new SelectCourseRes(student, SelectCourseRtnCode.SELECT_COURSE_SUCCESSFUL.getRtnmessage()); // 選課成功
	}

	private SelectCourseRes checkCourseConflict(Set<String> coursCode) {    //checkCourseConflict 課程衝突
...
//					boolean b = betweenExclude(oneClassGoDayInt, oneClassOutDayInt, twoClassGoDayInt, twoClassOutDayInt);
//					if (b ==true) {
...
//		return start1 >= end2 || end1 <= start2 ; //true= 不衝堂
	...
	public SelectCourseRes addClass(String studentId, Set<String> coursCode) {
		...
		if (!StringUtils.hasText(studentId) || CollectionUtils.isEmpty(coursCode)
				|| !StringUtils.hasText(coursecodeDelet)) {
...
//			studentCoursCode.setStudentClassCode(courscodeStr); // 將轉換型態的加選課程放回資訊欄
			...
			SelectCourseRes res = checkCourseNameConflictAndUnits(oldNewOpClass);
			if(res != null) {
				return res;
			}
			// ==============================================================
			// 不得衝堂
			SelectCourseRes courseConflictRes = checkCourseConflict(coursCode);
	...
	private SelectCourseRes checkCourseNameConflictAndUnits(Set<String> oldNewOpClass) {
		List<String> courseList = new ArrayList<>(); // 所有課程的清單
		Set<String> courseSet = new HashSet<>(); // 不得重複的課程清單
